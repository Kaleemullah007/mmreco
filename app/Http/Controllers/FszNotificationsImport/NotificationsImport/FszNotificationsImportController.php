...
namespace App\Http\Controllers\NotificationsImport;
...
class FszNotificationsImportController extends Controller
{
    ...
    {
        $entries = array();
        return view('notifications/view')->with('entries', $entries);
    }

    public function showEntries(Request $request)
    {
       
        $entries=FszNotification::with('notificationEntries')->whereHas('notificationEntries', function($q) use ($request){ 
            $q->whereBetween('booking_date',[$request->fromdate, $request->todate]);})->get();

        // $entries = DB::table('fsz_notifications')
        //     ->join('fsz_notification_enteries', 'fsz_notifications.id', '=', 'fsz_notification_enteries.notification_id')
